package dev.hawk0f.core.presentation.base

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import dev.hawk0f.core.domain.Either
import dev.hawk0f.core.domain.NetworkError
import dev.hawk0f.core.presentation.UIState
import dev.hawk0f.core.presentation.reset
...
abstract class BaseViewModel : ViewModel()
{
     ...
     */
    protected fun <T> Flow<Either<NetworkError, T>>.collectNetworkRequest(state: MutableStateFlow<UIState<T>>, resetStateAfterCollect: Boolean = false) = collectEither(state, resetStateAfterCollect) {
        UIState.Success(it)
    }
     ...
     * @receiver [collectEither]
     */
    protected fun <T, S> Flow<Either<NetworkError, T>>.collectNetworkRequest(state: MutableStateFlow<UIState<S>>, resetStateAfterCollect: Boolean = false, mapToUI: (T) -> S) = collectEither(state, resetStateAfterCollect) {
