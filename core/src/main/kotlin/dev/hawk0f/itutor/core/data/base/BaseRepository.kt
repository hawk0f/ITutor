...
import dev.hawk0f.itutor.core.BuildConfig
...
abstract class BaseRepository {
     ...
     */
    protected fun <T : DataMapper<S>, S> doNetworkRequestWithMapping(request: suspend () -> Response<T>): Flow<Either<NetworkError, S>> =
        doNetworkRequest(request) { body ->
     ...
     * @receiver [doNetworkRequest]
     */
    protected fun <T> doNetworkRequestWithoutMapping(request: suspend () -> Response<T>): Flow<Either<NetworkError, T>> =
        doNetworkRequest(request) { body ->
            Either.Right(body)
        }
     ...
     * @receiver [doNetworkRequest]
     */
    protected fun <T : DataMapper<S>, S> doNetworkRequestForList(request: suspend () -> Response<List<T>>): Flow<Either<NetworkError, List<S>>> =
        doNetworkRequest(request) { body ->
     ...
     * @receiver [doNetworkRequest]
     */
    protected fun doNetworkRequestUnit(request: suspend () -> Response<Unit>): Flow<Either<NetworkError, Unit>> =
        ...
        request().let {
            when {
                it.isSuccessful -> {
                    ...
                    val errorResource =
                        R.string::class.java.getDeclaredField(it.errorBody()!!.string())
                            .getInt(null)
