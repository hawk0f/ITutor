...
import dev.hawk0f.itutor.databinding.FragmentStudentBinding
import dev.hawk0f.itutor.student.presentation.ui.adapters.StudentAdapter
import dev.hawk0f.itutor.student.presentation.ui.viewmodels.StudentsViewModel
...
class StudentFragment : BaseFragment<StudentsViewModel, FragmentStudentBinding>(R.layout.fragment_student)
{
    ...
    private fun subscribeToStudents() = with(binding) {
        viewModel.studentState.collectAsUIState(state = {
            ...
            if (it.isEmpty())
            {
                noStudents.isVisible = true
                recyclerStudents.isVisible = false
            }
            else
            {
