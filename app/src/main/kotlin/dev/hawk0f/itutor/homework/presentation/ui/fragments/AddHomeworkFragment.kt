...
import dev.hawk0f.itutor.databinding.FragmentAddHomeworkBinding
import dev.hawk0f.itutor.homework.presentation.ui.viewmodels.AddHomeworkViewModel
...
class AddHomeworkFragment : BaseFragment<AddHomeworkViewModel, FragmentAddHomeworkBinding>(R.layout.fragment_add_homework)
{
    ...
    private fun subscribeToHomeworks() = with(binding) {
        viewModel.homeworksState.collectAsUIState(state = {
            ...
            {
                val lessonAdapter = ArrayAdapter(requireContext(), R.layout.dropdown_item, lessonsList.map { lessonStudent -> getString(dev.hawk0f.itutor.core.R.string.homework_lesson_info_input, lessonStudent.date.parseToFormat("d MMMM"), lessonStudent.startTime + " - " + lessonStudent.endTime) }.distinct())
                ...
                lessonDropDown.setOnItemClickListener { _, _, positionLesson, _ ->
                    ...
                    val lessonId = lessonsList.first { lesson -> getString(dev.hawk0f.itutor.core.R.string.homework_lesson_info_input, lesson.date.parseToFormat("d MMMM"), lesson.startTime + " - " + lesson.endTime) == lessonAdapter.getItem(positionLesson) }.lessonId
